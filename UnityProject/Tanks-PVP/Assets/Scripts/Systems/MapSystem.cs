...
public sealed class MapSystem : UpdateSystem {
    ...
    public bool CheckCollision(Vector2 pos, Vector2 size, BlockCollisionLayer collisionLayer, out CollisionInfo collisionInfo) {
        ...
        foreach (var blockEnt in collidableBlocksFilter) {
            ...
            //Vector2 subBlockPos;
            //Vector2 subBlockSize;

            /*
            if (collidableComponent.collisionSubBlocks == 15) {
                if (CheckAABBCollision(pos - size * 0.5f, pos + size * 0.5f, blockPos - blockSize * 0.5f, blockPos + blockSize * 0.5f, out _isMapBorderCollision)) {
                    flag = true;
                    break;
                }
            } else {
            ...
            continue;
            /*
            if(collidableComponent.collisionSubBlocks == 1) {
                // Bottom left
                subBlockPos = blockPos - blockSize * 0.25f;
                subBlockSize = blockSize * 0.5f;

                if (CheckAABBCollision(pos - size * 0.5f, pos + size * 0.5f, subBlockPos - subBlockSize*0.5f, subBlockPos + subBlockSize * 0.5f)) {
                    flag = true;
                    break;
                }
            } else if (collidableComponent.collisionSubBlocks == 2) {
                // Bottom right
                subBlockPos = blockPos + new Vector2(blockSize.x * 0.25f, - blockSize.y*0.25f);
                subBlockSize = blockSize * 0.5f;
            ...
            } else if (collidableComponent.collisionSubBlocks == 4) {
                // Top left
                subBlockPos = blockPos + new Vector2(-blockSize.x * 0.25f, blockSize.y * 0.25f);
                subBlockSize = blockSize * 0.5f;
            ...
            } else if (collidableComponent.collisionSubBlocks == 8) {
                // Top right
                subBlockPos = blockPos + blockSize * 0.25f;
                subBlockSize = blockSize * 0.5f;
            ...
            } else if (collidableComponent.collisionSubBlocks == 3) {
                // Both bottom
                subBlockPos = blockPos + new Vector2(0, -blockSize.y * 0.25f);
                subBlockSize = new Vector2(blockSize.x, blockSize.y*0.5f);

                if (CheckAABBCollision(pos - size * 0.5f, pos + size * 0.5f, subBlockPos - subBlockSize * 0.5f, subBlockPos + subBlockSize * 0.5f)) {
                    flag = true;
                    break;
                }
            } else if (collidableComponent.collisionSubBlocks == 5) {
                // Both left
                subBlockPos = blockPos + new Vector2(-blockSize.x*0.25f, 0);
                subBlockSize = new Vector2(blockSize.x * 0.5f, blockSize.y );

                if (CheckAABBCollision(pos - size * 0.5f, pos + size * 0.5f, subBlockPos - subBlockSize * 0.5f, subBlockPos + subBlockSize * 0.5f)) {
                    flag = true;
                    break;
                }
            } else if (collidableComponent.collisionSubBlocks == 12) {
                // Both top
                subBlockPos = blockPos + new Vector2(0, blockSize.y * 0.25f);
                subBlockSize = new Vector2(blockSize.x, blockSize.y * 0.5f);
            ...
            } else if (collidableComponent.collisionSubBlocks == 10) {
                // Both right
                subBlockPos = blockPos + new Vector2(blockSize.x * 0.25f, 0);
                subBlockSize = new Vector2(blockSize.x * 0.5f, blockSize.y);
            ...
            } else if (collidableComponent.collisionSubBlocks == 7) {
                // Except top right
                // Both left
                subBlockPos = blockPos + new Vector2(-blockSize.x * 0.25f, 0);
                subBlockSize = new Vector2(blockSize.x * 0.5f, blockSize.y);
            ...
            } else if (collidableComponent.collisionSubBlocks == 11) {
                // Except top left
                // Both right
                subBlockPos = blockPos + new Vector2(blockSize.x * 0.25f, 0);
                subBlockSize = new Vector2(blockSize.x * 0.5f, blockSize.y);
            ...
            } else if (collidableComponent.collisionSubBlocks == 14) {
                // Except bottom left
                // Both top
                subBlockPos = blockPos + new Vector2(0, blockSize.y * 0.25f);
                subBlockSize = new Vector2(blockSize.x, blockSize.y * 0.5f);
            ...
            } else if (collidableComponent.collisionSubBlocks == 13) {
                // Except bottom right
                // Both top
                subBlockPos = blockPos + new Vector2(0, blockSize.y * 0.25f);
                subBlockSize = new Vector2(blockSize.x, blockSize.y * 0.5f);
